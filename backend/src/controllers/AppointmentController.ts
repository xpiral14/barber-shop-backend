...
export default class AppointmentController {
  static async getAppointment(params: any) {
    const appointment = await Appointment.findOne(params);

    if (!appointment) throw new NotFound('Horário marcado');
  ...
  static async showByCostumer(req: any, res: any, next: any) {
    try {
      const { id } = req.params;
  ...
  static async showByBarber(req: any, res: any, next: any) {
    const { id } = req.params;
    const appointment = await AppointmentController.getAppointment({
      where: {
        id,
  ...
  static async cancel(req: any, res: any, next: any) {
    const { id } = req.params;
